...
def build_game_logs(report: dict[str, Any]) -> dict[str, Any]:
    ...
    merged_identity_logs = 0

    return {
        "schema_version": 4,
        "season": season,
        "source": {
            "type": "player_match_report_v8",
            "note": (
                "Identity normalization: v8 accepts only canonical person IDs. Bongé, Hönicke and El-Mahmoud are separate people; merged identities are rejected. "
...
def discover_report_file(project_root: Path | None) -> Path | None:
    ...
    candidates.extend([
        ...
        / "2025_discovery_v2"
        / "player_match_report_v8.json",
        cwd / "player_match_report_v8.json",
    ])
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not report_file or not report_file.exists():
        print(
            "ERROR: player_match_report_v8.json not found. "
