...
def _merge_identity(log: dict[str, Any], row: dict[str, Any]) -> None:
    ...
    if row.get("status") == "merged_identity":
        log["person_id"] = None
        log["canonical_name"] = None
        log["identity_status"] = "merged_identity"
...
def normalize_merged_identities(logs: list[dict[str, Any]]) -> int:
    """Keep explicit merged identities consistent across all games."""
    candidates_by_key: dict[str, set[int]] = defaultdict(set)

    for log in logs:
        player_key = str(log.get("player_key") or "")
        if not player_key.startswith("merged:"):
            continue
        for candidate in log.get("candidate_person_ids") or []:
            if isinstance(candidate, int):
                candidates_by_key[player_key].add(candidate)
    ...
    for log in logs:
        ...
        before = (
            log.get("person_id"),
            log.get("canonical_name"),
            log.get("identity_status"),
            tuple(log.get("candidate_person_ids") or []),
        )
        log["person_id"] = None
        log["canonical_name"] = None
        log["identity_status"] = "merged_identity"
        log["identity_source"] = "explicit_override"
        log["identity_assumption"] = "same_person_confirmed_by_project_review"
        log["candidate_person_ids"] = sorted(candidates_by_key[player_key])
...
def build_game_logs(report: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in report.get("rows") or []:
        ...
        if status not in {"auto", "merged_identity"}:
            continue
    ...
    normalize_merged_identities(logs)
    ...
    merged_identity_logs = sum(
        1 for log in logs if log.get("identity_status") == "merged_identity"
    )
    ...
    return {
        ...
        "season": season,
        "source": {
            "type": "player_match_report_v7",
            "note": (
                "Identity normalization: only explicit reviewed merged:* identities are merged; Hönicke/El-Mahmoud remain separate canonical players when the matcher can uniquely assign them. "
        ...
        "summary": {
            ...
            "logs_using_merged_identity": merged_identity_logs,
        },
        "game_logs": logs,
    }
...
def discover_report_file(project_root: Path | None) -> Path | None:
    ...
    candidates.extend([
        ...
        / "2025_discovery_v2"
        / "player_match_report_v7.json",
        cwd / "player_match_report_v7.json",
    ])
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not report_file or not report_file.exists():
        print(
            "ERROR: player_match_report_v7.json not found. "
    ...
    print(
        "Logs using merged identity: "
        f"{summary['logs_using_merged_identity']}"
    )
    print()
