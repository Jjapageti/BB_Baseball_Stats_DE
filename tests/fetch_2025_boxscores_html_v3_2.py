...
"""
BSM 2025 Boxscore HTML fetcher/parser v3.2
...
def load_candidate_leagues(path: Path) -> list[dict[str, Any]]:
    data = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(data, list):
        raise TypeError("candidate_leagues.json must contain a JSON list")
    return [row for row in data if isinstance(row, dict)]


def build_league_scope_map(leagues: list[dict[str, Any]]) -> dict[int, str]:
    result: dict[int, str] = {}
    for league in leagues:
        league_id = league.get("league_id")
        scope = league.get("scope")
        if isinstance(league_id, int) and isinstance(scope, str):
            result[league_id] = scope
    return result


def filter_batch_matches(
    matches: list[dict[str, Any]],
    league_scopes: dict[int, str],
    *,
    include_unpublished: bool = False,
) -> tuple[list[dict[str, Any]], dict[str, int]]:
    """
    v3.2 selection rule:
    - local_bsvbb: keep every published match in the league.
    - overregional_candidate: keep only published matches where at least one
      BSVBB club actually participates.
    - if a match belongs to both scopes, local_bsvbb wins.
    """
    selected: list[dict[str, Any]] = []
    stats = {
        "total_candidates": len(matches),
        "selected_local": 0,
        "selected_overregional": 0,
        "excluded_unpublished": 0,
        "excluded_overregional_without_bsvbb": 0,
        "excluded_unknown_scope": 0,
    }

    for match in matches:
        if not include_unpublished and match.get("statistics_published_flag") is not True:
            stats["excluded_unpublished"] += 1
            continue

        scopes: set[str] = set()
        for league_ref in match.get("candidate_leagues") or []:
            if not isinstance(league_ref, dict):
                continue
            league_id = league_ref.get("league_id")
            if isinstance(league_id, int):
                scope = league_scopes.get(league_id)
                if scope:
                    scopes.add(scope)

        if "local_bsvbb" in scopes:
            selected.append(match)
            stats["selected_local"] += 1
            continue

        if "overregional_candidate" in scopes:
            if match.get("bsvbb_clubs_in_match"):
                selected.append(match)
                stats["selected_overregional"] += 1
            else:
                stats["excluded_overregional_without_bsvbb"] += 1
            continue

        stats["excluded_unknown_scope"] += 1
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--league-file", type=Path, help="Path to 2025_discovery_v2/candidate_leagues.json")
    ...
    batch_hint = args.batch or args.candidate_file is not None or args.league_file is not None or args.limit is not None or args.probe_all
...
def main(argv: list[str] | None = None) -> int:
    ...
    league_file = args.league_file or candidate_file.with_name("candidate_leagues.json")
    if not league_file.exists():
        print(
            f"ERROR: candidate_leagues.json not found: {league_file}\n"
            "Use --league-file PATH explicitly.",
            file=sys.stderr,
        )
        return 1

    all_matches = load_candidate_matches(candidate_file)
    leagues = load_candidate_leagues(league_file)
    league_scopes = build_league_scope_map(leagues)
    matches, selection_stats = filter_batch_matches(
        all_matches,
        league_scopes,
        include_unpublished=args.probe_all,
    )

    selected_before_limit = len(matches)
    ...
    print(f"League file    : {league_file}")
    print(f"Candidates     : {selection_stats['total_candidates']}")
    print(f"Selected local : {selection_stats['selected_local']}")
    print(f"Selected over. : {selection_stats['selected_overregional']}")
    print(f"Excluded no-BSVBB overregional: {selection_stats['excluded_overregional_without_bsvbb']}")
    print(f"Excluded unpublished          : {selection_stats['excluded_unpublished']}")
    print(f"Eligible total : {selected_before_limit}")
    ...
    for idx, match in enumerate(matches, start=1):
        ...
        row["bsvbb_clubs_in_match"] = match.get("bsvbb_clubs_in_match")
