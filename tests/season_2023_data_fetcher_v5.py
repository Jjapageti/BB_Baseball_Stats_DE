#!/usr/bin/env python3
"""BSM 2023 BSVBB 전체 시즌 + BSVBB 참가 광역대회 데이터를 수집한다.
...
3. 동일 경기에서 league/second_league로 함께 연결된 그룹은 같은 logical league로
   병합한다(예: 2023 Landesliga DivA + DivB).
4. Baseball/Softball, 성인/청소년/Schüler/Coach Pitch/DM 등을 모두 허용한다.

출력
----
- <project>/bsm_season_data/2023/season_2023.json
- <project>/data/seasons/2023.json
- <project>/bsm_season_data/2023/fetch_report.txt
- <project>/bsm_season_data/2023/discovery.json
"""
...
# ---------------------------------------------------------------------------
SEASON = 2023
SCRIPT_VERSION = "2026-08-18-bsvbb-full-v5"
...
DISCOVERY_RELATIVE_DIRS = (
    Path("tests") / "bsm_season_data" / "2023_discovery_v2",
    Path("bsm_season_data") / "2023_discovery_v2",
)
...
LEVEL_PATTERNS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("1-bundesliga", ("1. baseball-bundesliga", "1. bundesliga", "1bl")),
    ...
    ("kinder", ("kinderliga", "kindbb")),
    ("tossball", ("tossball", "tossbb")),
...
def build_group_catalog_from_discovery(
    ...
    """Build exact 2023 BSVBB catalog from probe-v2 output.
...
def select_target_groups(catalog: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Probe-v2 already performed the 2023 allow-list decision; keep all 26 groups."""
    return [
        row
        for row in catalog
...
def find_discovery_inputs() -> tuple[Path, Path]:
    ...
    script_dir = Path(__file__).resolve().parent
    candidates.append(script_dir / "bsm_season_data" / "2023_discovery_v2")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build 2023 BSVBB full season data from probe-v2 discovery."
    )
    parser.add_argument(
        "--fetch",
        action="store_true",
