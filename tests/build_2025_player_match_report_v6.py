...
def build_match_report(
    *,
    ...
    # Cumulative totals can create seeds for role linkage, and role linkage can
    # in turn create enough known rows for another cumulative pass.
    for _ in range(8):
        ...
        if cumulative_now == 0 and role_now == 0:
            break

    boxscore_identity_resolved = resolve_remaining_by_boxscore_identity(report)
    ...
    report["summary"] = {
        ...
        "auto": auto,
        "boxscore_identity": boxscore_identity,
        ...
        "resolved_by_boxscore_identity": boxscore_identity_resolved,
        "auto_rate_percent": round((auto / total * 100) if total else 0.0, 2),
        "resolved_rate_percent": round(
            ((auto + boxscore_identity) / total * 100) if total else 0.0,
            2,
        ),
    }
    return report
...
def build_unresolved_summary(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in report.get("rows") or []:
        ...
        if status in {"auto", "boxscore_identity"}:
            continue
...
def make_text_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        f"auto: {s['auto']}",
        f"boxscore_identity: {s.get('boxscore_identity', 0)}",
        f"review: {s['review']}",
        ...
        f"resolved_by_boxscore_identity: {s.get('resolved_by_boxscore_identity', 0)}",
        ...
        "- boxscore_identity: same Berlin club + same Boxscore short name is treated as one player by explicit project assumption; candidate person IDs are retained.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path = output_dir / "player_match_report_v6.json"
    text_path = output_dir / "player_match_report_v6.txt"
    unresolved_json_path = output_dir / "player_match_unresolved_v6.json"
    unresolved_text_path = output_dir / "player_match_unresolved_v6.txt"
