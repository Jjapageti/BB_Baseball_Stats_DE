...
def enrich_dataset(
    ...
    summary["assigned_event_entries"] = (
        summary["assigned_event_entries_v1"] + len(resolved_v2)
    )
    summary["unresolved_event_entries"] = len(unresolved_v2)
    ...
    addition = (
        ...
        "resolved uniquely. v2 also uses game-stat hard constraints and exact "
        "league/team cumulative event chains for same-short-name collisions."
    )
...
def render_report(report: dict[str, Any]) -> str:
    s = report["summary"]
    lines = [
        "===== BSM Game Log Event Enrichment v2 =====",
        "",
        ...
        "Rows with no canonical game-log player remain unresolved; v2 does not synthesize fielding-only player rows.",
    ]
    return "\n".join(lines) + "\n"

def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Enrich 2025 canonical game logs from BSM raw HTML with v2 short-name resolution."
    )
...
def main() -> int:
    ...
    report_json = report_dir / "event_enrichment_report_v2.json"
    report_txt = report_dir / "event_enrichment_report_v2.txt"
    unresolved_json = report_dir / "event_enrichment_unresolved_v2.json"
    resolved_json = report_dir / "event_enrichment_resolved_v2.json"

    write_json(report_json, report)
    write_json(unresolved_json, report["unresolved"])
    write_json(resolved_json, report.get("resolved_v2") or [])
    ...
    print(f"Resolved v2 JSON: {resolved_json}")
