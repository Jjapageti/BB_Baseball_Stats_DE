#!/usr/bin/env python3
"""BSM BSVBB 전체 시즌 + BSVBB 참가 광역대회 데이터를 연도별로 수집한다.

이 버전은 Berlin Sluggers(Club-ID 492)만 기준으로 리그를 찾지 않는다.
먼저 probe-v2 discovery가 만든 candidate_leagues.json / candidate_matches.json을 입력으로 사용한다.
...
출력
----
- <project>/bsm_season_data/<season>/season_<season>.json
- <project>/data/seasons/<season>.json
- <project>/bsm_season_data/<season>/fetch_report.txt
- <project>/bsm_season_data/<season>/discovery.json
"""
...
SCRIPT_VERSION = "2026-08-17-bsvbb-multiseason-v1"
...
# probe-v2는 2025 local allow-list를 사용하므로, 다른 시즌에서 local BSVBB인데
# overregional_candidate로 떨어지는 리그만 여기서 명시적으로 보정한다.
# 2023은 discovery 결과를 확인한 뒤 필요 시 이 표에 추가하면 된다.
LOCAL_SCOPE_OVERRIDES_BY_SEASON: dict[int, set[str]] = {
    2024: {"JugABB", "LLBB", "TossBB"},
}

DISCOVERY_RELATIVE_DIRS: tuple[Path, ...] = ()


def configure_for_season(season: int) -> None:
    """Update all runtime paths for the requested season."""
    global SEASON, OUTPUT_DIR, SITE_OUTPUT, DISCOVERY_RELATIVE_DIRS

    SEASON = int(season)
    OUTPUT_DIR = OUTPUT_ROOT / str(SEASON)
    SITE_OUTPUT = PROJECT_ROOT / "data" / "seasons" / f"{SEASON}.json"
    DISCOVERY_RELATIVE_DIRS = (
        Path("tests") / "bsm_season_data" / f"{SEASON}_discovery_v2",
        Path("bsm_season_data") / f"{SEASON}_discovery_v2",
    )


configure_for_season(DEFAULT_SEASON)
...
def apply_scope_overrides(
    ...
    *,
    season: int,
    extra_local_acronyms: Iterable[str] | None = None,
) -> list[dict[str, Any]]:
    """Return a copied discovery list with season-specific local scope corrections.

    probe-v2 deliberately keeps its own discovery behavior. This fetcher only
    corrects known false overregional classifications after discovery so the
    original discovery artifacts remain untouched.
    """
    built_in = LOCAL_SCOPE_OVERRIDES_BY_SEASON.get(int(season), set())
    requested = set(extra_local_acronyms or [])
    local_acronyms = {
        text(value).casefold()
        for value in (*built_in, *requested)
        if text(value)
    }

    result: list[dict[str, Any]] = []
    for source in candidate_leagues:
        row = copy.deepcopy(source)
        acronym = text(row.get("league_acronym")).casefold()
        if acronym and acronym in local_acronyms:
            row["scope"] = "local_bsvbb"
            row["scope_override"] = {
                "season": int(season),
                "reason": "season_local_bsvbb_override",
            }
        result.append(row)
    return result
...
def build_group_catalog_from_discovery(
    ...
    """Build the selected-season BSVBB catalog from probe-v2 output.
...
def select_target_groups(catalog: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Keep all local BSVBB and BSVBB-related overregional discovery groups."""
    return [
        row
        for row in catalog
...
def find_discovery_inputs(discovery_dir: Path | None = None) -> tuple[Path, Path]:
    candidates: list[Path] = []

    if discovery_dir is not None:
        explicit = Path(discovery_dir)
        if not explicit.is_absolute():
            explicit = (PROJECT_ROOT / explicit).resolve()
    ...
    script_dir = Path(__file__).resolve().parent
    candidates.append(script_dir / "bsm_season_data" / f"{SEASON}_discovery_v2")
    ...
    raise FileNotFoundError(
        ...
        f"먼저 {SEASON} probe-v2 discovery를 생성하세요.\n"
        f"확인한 위치:\n{checked}"
    )
...
def run(
    *,
    ...
    print(f"[1/4] {SEASON} probe-v2 discovery 파일 읽기")
    candidate_league_path, candidate_match_path = find_discovery_inputs(discovery_dir)
    ...
    candidate_leagues = apply_scope_overrides(
        candidate_leagues,
        season=SEASON,
        extra_local_acronyms=extra_local_acronyms,
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build BSVBB full season data from probe-v2 discovery."
    )
    parser.add_argument(
        "--season",
        type=int,
        default=DEFAULT_SEASON,
        help=f"Season year to process (default: {DEFAULT_SEASON}).",
    )
    parser.add_argument(
        "--discovery-dir",
        type=Path,
        help="Explicit directory containing candidate_leagues.json and candidate_matches.json.",
    )
    parser.add_argument(
        "--local-acronym",
        action="append",
        default=[],
        help=(
            "Treat this league acronym as local_bsvbb for this run. "
            "Repeat the option for multiple acronyms; useful when validating a new season."
        ),
    )
    parser.add_argument(
        "--fetch",
...
def main(argv: list[str] | None = None) -> int:
    ...
    configure_for_season(args.season)
    try:
        return run(
            fetch=args.fetch,
            discovery_dir=args.discovery_dir,
            extra_local_acronyms=args.local_acronym,
        )
