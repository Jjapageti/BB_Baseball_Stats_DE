    ...
    *,
    ...
    for path in sorted(boxscore_files, key=lambda p: p.name):
        ...
        for occurrence in boxscore_occurrences(boxscore):
            ...
            if not is_berlin_boxscore_team(
                occurrence["team"],
                index,
                berlin_club_ids,
                set(league_ids),
...
def extract_batting_positions(raw_name: str) -> set[str]:
    """Return trailing BSM batting/fielding position tokens from a batting row."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path = output_dir / "player_match_report_v5.json"
    text_path = output_dir / "player_match_report_v5.txt"
    unresolved_json_path = output_dir / "player_match_unresolved_v5.json"
    unresolved_text_path = output_dir / "player_match_unresolved_v5.txt"
