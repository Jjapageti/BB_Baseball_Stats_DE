...
SAME_PERSON_OVERRIDES = [
    {
        ...
        "player_key": "merged:2025:club:604:name:bonge-j",
    },
]
...
def build_match_report(
    *,
    ...
    # Evidence passes can seed one another. Explicit same-person overrides are
    # already removed from ambiguity before these passes.
    for _ in range(8):
    ...
    report["summary"] = {
        ...
        "resolved_rate_percent": round(
            ((auto + merged_identity) / total * 100) if total else 0.0,
            2,
        ),
    }
    return report
...
def apply_same_person_overrides(report: dict[str, Any]) -> int:
    ...
    Only Bongé J. (71909/73795) is merged. Hönicke L. and El-Mahmoud M.
    are intentionally excluded because those abbreviations represent
    different people.
    """
...
def resolve_configured_season_total_splits(
    ...
    """Split only the reviewed Hönicke/El-Mahmoud abbreviation collisions.
...
def resolve_configured_pitching_record_chains(
    ...
    Used only for the configured Hönicke/El-Mahmoud split identities. Same-game
...
def build_unresolved_summary(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in report.get("rows") or []:
        ...
        if status in {"auto", "merged_identity"}:
            continue
...
def make_text_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "- merged_identity: explicit reviewed same-person override (currently Bongé J. only); canonical IDs are retained as aliases but no single person_id is chosen.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path = output_dir / "player_match_report_v7.json"
    text_path = output_dir / "player_match_report_v7.txt"
    unresolved_json_path = output_dir / "player_match_unresolved_v7.json"
    unresolved_text_path = output_dir / "player_match_unresolved_v7.txt"
