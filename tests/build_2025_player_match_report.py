...
def build_canonical_index(season_data: dict[str, Any]) -> dict[str, Any]:
    ...
    contexts_by_person: dict[int, list[dict[str, Any]]] = defaultdict(list)
        ...
        for role in ("batting", "pitching"):
            for stat in league.get(role) or []:
                ...
                context_key = (
                    person_id,
                    context["league_key"],
                    context["league_entry_id"],
                    tuple(context["group_ids"]),
                    tuple(context["team_aliases"]),
                )
                if context_key not in seen_contexts:
                    contexts_by_person[person_id].append(context)
                    seen_contexts.add(context_key)

    return {
        "people": people,
        "contexts_by_person": dict(contexts_by_person),
...
def _person_has_group(
    ...
    for context in index["contexts_by_person"].get(person_id, []):
        if league_ids.intersection(context.get("group_ids") or []):
            return True
    return False
...
def _person_has_team(
    ...
    if not team_key:
        return False
    for context in index["contexts_by_person"].get(person_id, []):
        if team_key in set(context.get("team_aliases") or []):
            return True
    return False
...
def _person_has_group_and_team(
    ...
    for context in index["contexts_by_person"].get(person_id, []):
        if (
            league_ids.intersection(context.get("group_ids") or [])
            and team_key in set(context.get("team_aliases") or [])
        ):
            return True
    return False
...
def match_player(
    *,
    ...
    team_candidates = sorted(set(team_candidates))
    ...
    group_candidates = sorted(set(group_candidates))
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path = output_dir / "player_match_report.json"
    text_path = output_dir / "player_match_report.txt"
